...
**Roc Scripts** This example also demonstrates how Roc applications can be used as scripts. The file starts with a `#!/usr/bin/env roc` which tells the operating system (on macOS or linux) to run this file as a script. This can then be done with the command `./hello-json.roc`. Note that you may need to make the file executable using `chmod +x hello-json.roc`
